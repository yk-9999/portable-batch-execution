...
def _aggregate_capacity(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "holding_days_median": float(np.nanmedian(hd)) if np.isfinite(hd).any() else float("nan"),
        "mae_median": float(np.nanmedian(mae)) if np.isfinite(mae).any() else float("nan"),
        "mfe_median": float(np.nanmedian(mfe)) if np.isfinite(mfe).any() else float("nan"),
    }
...
def _summarise_residual(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    diags = [
        r
        for r in rows
        if r.get("residual_market_beta") is not None or r.get("residual_sector_beta") is not None
    ]
    if not diags:
        return {"n": 0}
...
def _apply_scenario(record: NormalizedTradePathRecord, scen: TradePathScenarioParams) -> dict[str, Any]:
...
def _summarise_event(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for r in records:
        if r.get("status") == "cancelled":
            ...
            censored += 1
    gross = np.array([r["gross_pnl"] for r in rows], dtype=float) if rows else np.array([])
    ...
    return {
        ...
        "breakeven_round_trip_bps_median": float(
            np.nanmedian(
                [_breakeven_round_trip_bps(r["gross_pnl"], r["traded_notional"]) for r in rows]
            )
        )
        if rows
        else float("nan"),
