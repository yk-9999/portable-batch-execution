...
class HttpPrivateDataPlane:
    def __init__(
        self, base_url: str, bearer_token: str, *, client: httpx.Client | None = None
    ):
    ...
    def from_environment(cls) -> HttpPrivateDataPlane:
        base_url, token = (
            os.environ.get("PBE_PRIVATE_DATA_PLANE_BASE_URL"),
            os.environ.get("PBE_PRIVATE_DATA_PLANE_BEARER_TOKEN"),
        )
    ...
    def _request(
        ...
        try:
            ...
            raise PrivateDataPlaneError(
                f"private data plane {operation} failed"
            ) from exc
        if response.status_code >= 400:
            raise PrivateDataPlaneError(
                f"private data plane {operation} failed with HTTP {response.status_code}"
            )
        return response
    ...
    def _json(response: httpx.Response, operation: str) -> Any:
        try:
            ...
            raise PrivateDataPlaneError(
                f"private data plane {operation} returned invalid JSON"
            ) from exc
    ...
    def resolve_wave(self, run_id: str, wave_id: str) -> dict[str, Any]:
        payload = self._json(
            self._request(
                "resolve wave",
                "GET",
                f"/v1/runs/{self._part(run_id, 'run_id')}/waves/{self._part(wave_id, 'wave_id')}",
            ),
            "resolve wave",
        )
        if not isinstance(payload, dict):
            raise PrivateDataPlaneError(
                "private data plane resolve wave returned invalid JSON"
            )
        return payload
    ...
    def read(self, ref: ArtifactRef) -> bytes:
        return self._request(
            "read artifact",
            "GET",
            f"/v1/artifacts/{self._part(ref.object_id, 'artifact object_id')}/content",
        ).content

    @staticmethod
    def _resolve_bounded_stream_size(ref: ArtifactRef, response: httpx.Response) -> int:
    ...
    def write(self, data: bytes, media_type: str | None = None) -> ArtifactRef:
        ...
        try:
            ...
            raise PrivateDataPlaneError(
                "private data plane write artifact returned invalid reference"
            ) from exc
    ...
    def exists(self, ref: ArtifactRef) -> bool:
        try:
            response = self._client.request(
                "HEAD",
                f"{self.base_url}/v1/artifacts/{self._part(ref.object_id, 'artifact object_id')}/content",
            )
    ...
    def append_attempt(self, record: ShardAttemptRecord) -> None:
        self._request(
            "append attempt",
            "POST",
            f"/v1/runs/{self._part(record.logical_run_id, 'run_id')}/attempts",
    ...
    def read_attempts(self, run_id: str) -> tuple[ShardAttemptRecord, ...]:
        payload = self._json(
            self._request(
                "read attempts",
                "GET",
                f"/v1/runs/{self._part(run_id, 'run_id')}/attempts",
            ),
            "read attempts",
        )
        try:
            return tuple(ShardAttemptRecord.model_validate(item) for item in payload)
        except (TypeError, ValueError) as exc:
            raise PrivateDataPlaneError(
                "private data plane read attempts returned invalid contracts"
            ) from exc
    ...
    def read_manifest(self, run_id: str) -> RunManifest | None:
        try:
            response = self._client.request(
                "GET",
                f"{self.base_url}/v1/runs/{self._part(run_id, 'run_id')}/manifest",
            )
        except httpx.HTTPError as exc:
            raise PrivateDataPlaneError(
                "private data plane read manifest failed"
            ) from exc
        if response.status_code == 404:
            return None
        if response.status_code >= 400:
            raise PrivateDataPlaneError(
                f"private data plane read manifest failed with HTTP {response.status_code}"
            )
        try:
            return RunManifest.model_validate(response.json())
        except ValueError as exc:
            raise PrivateDataPlaneError(
                "private data plane read manifest returned invalid contracts"
            ) from exc
    ...
    def write_next_manifest(
        ...
        try:
            response = self._client.request(
                "PUT",
                f"{self.base_url}/v1/runs/{self._part(manifest.logical_run_id, 'run_id')}/manifest",
                json=manifest.model_dump(mode="json"),
                headers={"If-Match": str(expected_revision)},
            )
        except httpx.HTTPError as exc:
            raise PrivateDataPlaneError(
                "private data plane write manifest failed"
            ) from exc
        if response.status_code == 409:
            raise RevisionConflictError("private data plane manifest revision conflict")
        if response.status_code >= 400:
            raise PrivateDataPlaneError(
                f"private data plane write manifest failed with HTTP {response.status_code}"
            )
        try:
            return RunManifest.model_validate(response.json())
        except ValueError as exc:
            raise PrivateDataPlaneError(
                "private data plane write manifest returned invalid contracts"
            ) from exc
