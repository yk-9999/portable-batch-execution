...
def validate_wave_descriptor_payload(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if job.operation_params.get("transport_profile") != "hf_bucket_direct":
        raise HfDirectWaveError("transport_profile mismatch")
    return {
...
def execute_hf_direct_wave(
    *,
    ...
    if not bucket_prefix and shards:
        first_path = parse_hf_object_uri(shards[0].input_refs[0].uri)
        match = re.match(r"^(?P<prefix>.+/)normalized/", first_path)
        if match is None:
            raise HfDirectWaveError("cannot derive bucket_prefix from shard input path")
    ...
    manifest_bytes = json.dumps(manifest_body, sort_keys=True, separators=(",", ":")).encode(
        "utf-8"
    )
