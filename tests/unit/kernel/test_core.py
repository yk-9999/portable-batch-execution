...
def test_stale_success_is_missing_and_current_success_is_canonical():
    ...
    shard = plan_shards(
        "run",
        RangeSpec(kind="index", start=0, end=1),
        1,
        ShardCorrectnessSpec(),
        (ref,),
        "current",
    ...
    canonical, missing, duplicate = completeness(
        {shard.shard_id}, [stale, current], [shard]
    )
...
def test_stale_attempts_do_not_exhaust_current_retry_budget():
    ...
    assert retryable_shards(
        [shard], [stale], ExecutionPolicy(max_parallel=1, max_attempts_per_shard=1)
    ) == (shard,)
...
def test_exhausted_shards_counts_initial_attempt_and_three_retries_only():
    ...
    def attempt(attempt_id, status="failed", digest="current"):
        return ShardAttemptRecord(
            ...
            started_at=now,
            finished_at=now,
            failure="failed" if status == "failed" else None,
        )
    ...
    assert exhausted_shards([shard], [attempt(str(i)) for i in range(4)], policy) == (
        shard,
    )
    assert (
        exhausted_shards(
            [shard],
            [attempt(str(i)) for i in range(3)] + [attempt("success", "succeeded")],
            policy,
        )
        == ()
    )
    assert (
        exhausted_shards(
            [shard],
            [attempt(str(i), digest="stale") for i in range(9)]
            + [attempt(str(i)) for i in range(3)],
            policy,
        )
        == ()
    )
