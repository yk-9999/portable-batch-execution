...
def test_scores_match_sklearn_reference_including_zero_vector():
    left = [
        _row("l1", [1.0, 0.0, 2.0]),
        _row("l2", [0.0, 0.0, 0.0]),
        _row("l3", [-1.5, 2.0, 0.5]),
    ...
    expected = cosine_similarity(left_matrix, right_matrix)
    ...
    assert np.allclose(result["scores"], expected.tolist(), rtol=0, atol=1e-12)
    assert result["scores"][1] == [0.0, 0.0]
