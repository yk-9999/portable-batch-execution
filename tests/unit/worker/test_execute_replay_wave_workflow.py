...
RESOLVE_SCRIPT = REPO_ROOT / ".github" / "scripts" / "resolve_execute_wave_dependency_profile.py"
PYPROJECT = REPO_ROOT / "pyproject.toml"


def _load_resolve_module():
    spec = importlib.util.spec_from_file_location("resolve_execute_wave_dependency_profile", RESOLVE_SCRIPT)
...
def test_execute_wave_remains_dispatch_workflow(workflow):
    ...
    assert not (REPO_ROOT / ".github" / "workflows" / "execute-replay-wave.yml").exists()
...
def test_execute_wave_replay_path_skips_heavy_setup(workflow):
    ...
    ffmpeg_block = workflow.split("name: Install FFmpeg", 1)[1].split("uv run python", 1)[0]
...
def test_resolve_profile_uses_private_run_wave_endpoint_with_bearer():
    ...
    assert captured["url"] == "https://plane.example/v1/runs/opaque-run/waves/opaque-wave"
...
def test_resolve_profile_main_does_not_print_bearer_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(urllib.request, "urlopen", lambda request, timeout=60: _Response())
    resolve = _load_resolve_module()
