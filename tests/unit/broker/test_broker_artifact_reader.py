...
def test_http_reader_size_mismatch_fails_closed(tmp_path):
    ...
    token_path = tmp_path / "token"
    token_path.write_text("t", encoding="utf-8")
    client = httpx.Client(transport=httpx.MockTransport(lambda _r: httpx.Response(200, content=payload)))
...
def test_accepts_loopback_http_and_https_origins():
    assert validate_artifact_read_base_url("http://127.0.0.1:18090") == "http://127.0.0.1:18090"
    assert validate_artifact_read_base_url("https://data.example.test") == "https://data.example.test"
